    ...
    def parse_tools(self, text: str) -> Optional[List[ToolCall]]:
        ...
        try:
            ...
            for match in matches:
                ...
                logger.info(f"parse_tools: found function match - name='{function_name}'")
                ...
                logger.info(f"parse_tools: extracted arguments for {function_name}: {arguments}")
            ...
            if tool_calls:
                logger.info(f"parse_tools: successfully parsed {len(tool_calls)} tool call(s)")
            else:
